...
| `scripts/compute_wer.py` | Not started | Compute per-meeting WER by aligning Whisper output against AMI gold transcripts |
...
**2. Compute WER per meeting**
- Script needed: `scripts/compute_wer.py` — align Whisper output against AMI gold transcripts using `jiwer` or similar, write per-meeting WER to `data/analysis/wer_per_meeting.csv`.
- Required for WER-stratified tables (RQ1, RQ4).
